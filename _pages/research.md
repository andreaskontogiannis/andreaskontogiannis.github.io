...
- **Learning under hidden-convex structure**: We study online learning with nonconvex losses that become convex under an unknown nonlinear reparameterization. In [arXiv 2026](https://arxiv.org/abs/2605.26373), we prove that Online Gradient Descent achieves optimal regret \(O(\sqrt{T})\) in the adversarial setting, matching the classical rate for online convex optimization; thereby improving over the previous established analysis that achieved \(O(T^{2/3})\) regret. We also characterize the geometric compatibility needed for this equivalence, show a linear-regret barrier when it fails, and extend the framework to one-point bandit feedback with \(O(T^{3/4})\) expected regret.
